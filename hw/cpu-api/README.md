...
Q2.
As the percentage increase, the process tree will become deeply nested.
...
Q4.
Orphaned processes will become child processes of `init` process.
Final process tree is a (b, d, e).
`-R` reparents the orphaned processes to local parent.
Final process tree is a (b (d e)).
...
Final process tree: a (c (d) e).

Q6.
In case, there is no process exit, it is possible to tell what actions took place.
If there are 2 actions and only a remains, there is only one possibility: a+b, b-.
However, if there are more than 2 actions and there is at least one exit, it is hard to tell actions accurately.
